...
% logarithmic
% L_vector = 5;
TCR_vector = [0, 5, 10]; % in dB

LOS_vector = get_LOS_vector(config);

for idx_L = 1:length(TCR_vector)
    %config.L = L_vector(idx_L);
    config.TCR = TCR_vector(idx_L);
    ...
    % These weights can be updated in the Adaptive Waveform Design (AWD) module
    %A = eye(config.L); % LxL complex diagonal matrix ontaining the transmitted weights
    % We get the coefficients in A from the TCR value
