...
clear all,
clc,
config = load_config(3); % Load configuration
...
[pfa, pd, pfa_opt, pd_opt] = get_ROC_from_config_IM(config, LOS_vector, T_rangecell)

% Plot ROC for this configuration
semilogx(pfa(:,1), pd(:,1), 'displayname', ['AWD disabled + AE'], ...
    'linewidth', 1.5, 'color', config.plot_color(2), ...
    'linestyle', config.plot_linestyle(2));
hold on,
semilogx(pfa_opt(:,1), pd_opt(:,1), 'displayname', ['AWD enabled + AE'], ...
